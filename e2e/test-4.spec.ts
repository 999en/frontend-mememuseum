...
test('Upload meme', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Dai un titolo al tuo meme...' }).fill('Titolo test 4');
  await page.getByRole('textbox', { name: 'funny, meme, gaming, cat,' }).click();
  await page.getByRole('textbox', { name: 'funny, meme, gaming, cat,' }).fill('TagTest1, TagTest2, TagRicercaTest11');
  ...
  await page.getByRole('img', { name: 'Titolo test 4' }).first().click();
