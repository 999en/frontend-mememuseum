...
test('Modifica test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Dai un titolo al tuo meme...' }).fill('Titolo test modificato');
  ...
  await expect(page.locator('div').filter({ hasText: 'AngularMasterTitolo test modificato #' }).nth(3)).toBeVisible();
  await page.getByRole('img', { name: 'Titolo test modificato' }).click();
